...
- 已完成边界：history 列表已支持 cursor 分页，详情已拆分为元信息 + 回放分页读取
- 仍然缺失：筛选、 richer review 生成策略

## 输入输出契约

- 输入：用户标识、会话标识、筛选条件
- 输出：历史会话列表、单会话详情、复盘信息
- 关键字段：`session_id`、`messages`、`cursor`、场景、角色、反馈
...
- 还没覆盖的风险：筛选、字段扩展和 richer review 生成策略

## 阻塞项

- 还没有筛选和 richer review 生成策略
...
- `2026-04-08`：history 列表改为 cursor 分页，详情改为元信息读取，新增 `/history/sessions/{session_id}/messages` 回放分页接口
